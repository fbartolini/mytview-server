<script lang="ts">
	...
	let {
		...
		srcUrl?: string | null;
		hlsUrl?: string | null;
		// Subtitle sidecars the server found next to the media file, in ITS order (contract
		// §subtitles). Rendered as <track>; nothing is fetched or generated by the client.
		subtitles?: { lang: string | null; label: string; kind: 'captions' | 'subtitles'; url: string }[];
		// Caption appearance, server-owned per user (/api/v1/me) so it follows them across devices.
		// Audio tracks in the file. A non-default choice is served by the SERVER (HTML5 video can't
		// switch audio inside a container), so picking one switches the source to HLS with `&a=`.
	...
	// Which audio track is playing. null = whatever the file leads with (its `default`), which is what
	// direct play gives us; a number means we asked the server for that stream over HLS.
	let audioIndex = $state<number | null>(null);
	let audioOpen = $state(false);
	// The label of the caption track currently showing — the browser's menu doesn't say.
	let activeCue = $state<string | null>(null);
	const hlsBase = $derived(hlsUrl ?? `/hls/v/${encodeURIComponent(id)}/index.m3u8`);
	// `&a=<stream index>` asks the server to encode THAT audio track — the only way to change audio,
	// since the browser plays whichever stream the container leads with and offers no way to switch.
	const hlsSrc = $derived(
		audioIndex == null ? hlsBase : hlsBase + (hlsBase.includes('?') ? '&' : '?') + `a=${audioIndex}`
	);
	...
	// Remembered subtitle CHOICE (a language tag, or '' for off). A remembered choice is not the same
	// as a default: nothing turns on until the viewer asks once, and then it follows them between
	// videos — which is the whole point for someone who always wants captions.
	const SUBS_KEY = 'myt-view:subtitleLang';
	const activeAudio = $derived(
		audioIndex != null
	...
	/**
	 * Switch audio track. The browser cannot do this — it plays whichever stream the container leads
	 * with — so the server re-encodes with the chosen one and we move to that HLS stream, resuming at
	 * the same second. Choosing the file's default returns to direct play, which costs the server
	 * nothing.
	 */
	function selectAudio(index: number | null) {
		audioOpen = false;
		if (index === audioIndex) return;
		const at = videoEl?.currentTime ?? 0;
		audioIndex = index;
		const isDefault = index == null || !!audioTracks.find((t) => t.index === index)?.default;
		if (isDefault && !onHls) {
			// The file's own default is what direct play already gives — no transcode needed.
			retryFromPos = at;
			videoEl?.load();
			return;
		}
	...
	/** Re-apply the remembered language to whatever this video happens to offer. */
	function restoreSubChoice() {
		if (!subtitles.length) return;
		let want: string | null = null;
		try {
			want = typeof localStorage !== 'undefined' ? localStorage.getItem(SUBS_KEY) : null;
		} catch {
			want = null;
		}
		if (!want) return; // never chosen, or explicitly off — leave captions off
		const i = subtitles.findIndex((t) => t.lang === want);
		const tracks = videoEl?.textTracks;
		if (i < 0 || !tracks) return;
		for (let t = 0; t < tracks.length; t++) tracks[t].mode = t === i ? 'showing' : 'disabled';
	}

	/** Remember the language whenever the viewer changes it in the BROWSER's own captions menu —
	 *  the only signal we get is the track list changing mode, so listen for it. */
	function onTrackChange() {
		...
		for (let t = 0; t < tracks.length; t++) {
			if (tracks[t].mode === 'showing') {
				chosen = subtitles[t]?.lang ?? '';
				activeCue = subtitles[t]?.label ?? null;
			}
		}
		if (typeof localStorage === 'undefined') return;
		try {
			localStorage.setItem(SUBS_KEY, chosen);
		} catch {
	...
	let hlsNetFails = 0; // consecutive FATAL network errors on the HLS path (see the ERROR handler)
	// Where a re-sourced HLS stream should resume: the element's clock, or the last ontimeupdate if a
	// failed load has already reset it to 0.
	const resumePos = () => (videoEl && videoEl.currentTime > 0 ? videoEl.currentTime : lastPos);
	...
	async function onError() {
		...
		if (onHls) {
			...
			// Native (Safari/iOS) HLS has no recovery hook, but it hits the same GC'd-session wall: re-mint
			// the playlist (bounded by the shared budget) before conceding.
			if (hlsRecoveries++ < MAX_HLS_RECOVERIES) return void switchToHls(resumePos(), true);
			giveUpHls(); // recovery exhausted → a simple, honest error
			return;
		}
		const code = err?.code;
	...
	// `recovery` = re-minting a failed session from inside the recovery ladder. It must NOT refill the
	// budget: resetting it here made the MAX_HLS_RECOVERIES cap unreachable, so a stream that never
	// plays looped forever, minting a server session per attempt until the server's session table was
	// full and every playlist 404'd (field 2026-09-13: 4K HEVC MKV, `retries=0` on every line). Only a
	// fresh start (fail-open, audio switch, Try again) or real playback (onPlaying) refills it.
	async function switchToHls(resumeAt = -1, recovery = false) {
		if (resumeAt >= 0) retryFromPos = resumeAt; // onLoaded seeks here once the new stream is ready
		// Already streaming? An audio change means a DIFFERENT encode, so tear the session down and
		// re-attach rather than trying to swap a track inside a running one.
		if (onHls) {
		...
		codecFail = false;
		hlsFailed = false;
		hlsNetFails = 0; // a fresh session → fresh network-failure streak (the recovery budget stays spent)
		if (!recovery) hlsRecoveries = 0;
		...
		try {
			...
			inst.on(HlsCtor.Events.ERROR, (_e, data) => {
				...
				if (data.type === HlsCtor.ErrorTypes.NETWORK_ERROR) {
					// TWO network failures in a row is not a blip — it's a DEAD SESSION. Pause longer than
					// HLS_SESSION_TTL and the server destroys the session behind this playlist; the sid is
					// random, so it cannot be resurrected and every segment URL in hand 503s forever, which
					// startLoad() can only re-hit (field 2026-08-26: paused a while, resumed, played ~5s off
					// the buffer, then "couldn't play this video"). Re-fetch the PLAYLIST instead — that mints
					// a fresh session, and onLoaded seeks back to where we were.
					if (hlsNetFails++ === 0) inst.startLoad();
					else setTimeout(() => switchToHls(resumePos(), true), 0); // never tear down inside hls.js's own handler
	...
	function onLoaded() {
		restoreSubChoice();
		videoEl?.textTracks?.addEventListener?.('change', onTrackChange);
	...
	function onTime() {
		...
		const now = Date.now();
		// Positions flow REGARDLESS of the watched flag — the server owns it (contract §Rewatch,
		// 2026-08-18): a watched video reporting a real mid-video position flips back to in-progress
		// with the offset kept; at/past the threshold it stays (or becomes) watched. The old
		// `!watched` gate here starved that rule of input — a rewatch sent NOTHING, so its progress
		// silently vanished on this one client while every native reported normally.
	...
	function saveNow() {
		// No `!watched` gate — same reason as the throttled write above (the exit save is exactly the
		// one that captures "skipped to the middle, watched a bit, left").
		if (videoEl && !videoEl.ended) post({ position: videoEl.currentTime });
	}
...
<!-- data-cue-* are what the :global ::cue rules below key on — they must sit on an ANCESTOR of the
     <video> (the cue lives in its shadow tree). Missing until 2026-08-24: the rules existed but no
     element carried the attributes, so caption prefs never applied on web. -->
<div class={frameClass} data-cue-size={subtitleSize} data-cue-color={subtitleColor}>
	<!-- svelte-ignore a11y_media_has_caption -->
	<!-- Subtitle tracks are declared but never `default`: captions are a user choice, and forcing them
	     on is as wrong as hiding them. The browser's own CC control (part of `controls`) drives them,
	     which also means the picker is keyboard-accessible and screen-reader-labelled for free. -->
	<video
		bind:this={videoEl}
		controls
		autoplay
...
<div class="mt-2.5 flex flex-wrap items-center gap-3 font-mono text-xs">
	...
	<div class="ml-auto flex items-center gap-3">
		<!-- AUDIO. The browser has no control for this at all: it plays whichever stream the container
		     leads with. So the picker is ours, and choosing a non-default track asks the server to
		     encode that stream (a transcode) — hence the note on those entries. -->
		{#if audioTracks.length > 1}
			<div class="relative">
				<button
					...
					class="rounded-md border border-line px-2.5 py-1 text-muted transition-colors hover:text-base-content"
					title="Audio track"
				>
					audio · {activeAudio?.label ?? 'default'}
				</button>
				{#if audioOpen}
					<div role="menu" class="absolute right-0 bottom-full z-20 mb-1 min-w-56 overflow-hidden rounded-lg border border-line bg-base-200 py-1 shadow-xl">
						{#each audioTracks as t (t.index)}
							<button
								role="menuitemradio"
								aria-checked={activeAudio?.index === t.index}
								onclick={() => selectAudio(t.default ? null : t.index)}
								class="block w-full px-3 py-1.5 text-left hover:bg-base-100 {activeAudio?.index === t.index
									? 'text-primary'
									: 'text-muted'}"
							>
								{t.label}{t.default ? ' · default' : ''}
								{#if !t.default}<span class="ml-1 text-faint">(re-encodes)</span>{/if}
							</button>
						{/each}
					</div>
				{/if}
		...
		<!-- CAPTIONS. Selection belongs to the browser's own ⋮ menu (keyboard-accessible and
		     screen-reader-labelled for free); what that menu cannot say is "this video has none", and
		     it doesn't tell you at a glance which track is running. So we state both here. -->
		<span
			class={activeCue ? 'text-primary' : 'text-faint'}
			title={subtitles.length
				? `Available: ${subtitles.map((t) => t.label).join(', ')} — choose from the player’s ⋮ menu`
				: 'This video has no subtitle track'}
		>
			{#if activeCue}
				CC · {activeCue}
			{:else if subtitles.length}
				CC · off ({subtitles.length})
			{:else}
				CC · none
			{/if}
		</span>
...
<style>
	/*
	 * Caption appearance. ::cue is the only handle a browser gives us on subtitle rendering, and it
	 * must be :global — the cue lives in the <video>'s shadow tree, where Svelte's scoping cannot
	 * reach. Sizes are PERCENTAGES of the browser's own default, which is itself relative to the
	 * video size: that keeps captions proportionate whether the video is a small inline player or
	 * full screen, where a fixed px size would be unreadable at one end and enormous at the other.
	 */
	:global([data-cue-size='small'] ::cue) {
		font-size: 80%;
	}
	:global([data-cue-size='medium'] ::cue) {
		font-size: 115%;
	}
	:global([data-cue-size='large'] ::cue) {
		font-size: 165%;
	}
	:global([data-cue-color='yellow'] ::cue) {
		color: #ffe14d; /* the broadcast-caption yellow — easier to separate from a busy picture */
	}
</style>
