...
export const GET: RequestHandler = async ({ params, request, url, locals }) => {
	...
	if (!verifyMedia('poster', params.id, url) && !canSeeChannel(locals.user, params.id))
		throw error(404);
	const r = db().prepare('SELECT poster_path FROM channels WHERE id = ?').get(params.id) as
		| { poster_path: string | null }
		| undefined;
	if (!r) throw error(404);
	const { absPath, stat } = await resolveInMediaRoot(r.poster_path);
