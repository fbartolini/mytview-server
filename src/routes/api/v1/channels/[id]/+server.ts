...
import { getChannel, directPlayMap, verticalMap, nextEpisode } from '$lib/server/queries';
...
// One channel + its videos (newest first; a series returns all episodes in season/episode order), each
// carrying `directPlay`/`isVertical` flags, a signed thumb URL, and this user's watch state; the channel
// carries signed poster/fanart + `isHidden`. For a series, `nextEpisode` is the server-owned continue
// pointer (first unwatched episode in order, or null). `?watched=1` reveals watched videos (hidden by
// default, like the feed). Auth: bearer token or cookie.
